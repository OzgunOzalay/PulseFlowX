...
class ROIAnalyzer:
    ...
    def download_atlas_masks(self, roi_name, hemisphere='bilateral'):
        ...
        # Define Harvard-Oxford atlas specifications
        atlas_specs = {
            'amygdala': {
                'left': 'Harvard-Oxford subcortical: Left Amygdala',
                'right': 'Harvard-Oxford subcortical: Right Amygdala',
                'bilateral': 'Harvard-Oxford subcortical: Amygdala'
            },
            'insula': {
                'left': 'Harvard-Oxford cortical: Left Insular Cortex',
                'right': 'Harvard-Oxford cortical: Right Insular Cortex', 
                'bilateral': 'Harvard-Oxford cortical: Insular Cortex'
            },
            'prefrontal_cortex': {
                'left': 'Harvard-Oxford cortical: Left Frontal Pole',
                'right': 'Harvard-Oxford cortical: Right Frontal Pole',
                'bilateral': 'Harvard-Oxford cortical: Frontal Pole'
            },
            'hippocampus': {
                'left': 'Harvard-Oxford subcortical: Left Hippocampus',
                'right': 'Harvard-Oxford subcortical: Right Hippocampus',
                'bilateral': 'Harvard-Oxford subcortical: Hippocampus'
            }
        }
        ...
        output_file = atlas_dir / f"{roi_name}_{hemisphere}_mask.nii.gz"
        
        # Use AFNI's whereami to get atlas mask
        cmd = f"whereami -atlas Harvard-Oxford -mask_atr_region '{atlas_label}' -prefix {output_file}"
        
        try:
            subprocess.run(cmd, shell=True, check=True, capture_output=True)
            self.logger.info(f"✓ Created atlas mask: {output_file}")
            return output_file
    ...
    def extract_roi_values(self, subject_id, data_file, roi_mask, roi_name, hemisphere):
        ...
        try:
            ...
            self.logger.info(f"✓ Extracted ROI values: mean={summary['mean']:.4f}, std={summary['std']:.4f}")
            return summary
    ...
    def analyze_subject_roi(self, subject_id, roi_name='amygdala', hemisphere='bilateral'):
        """Complete ROI analysis for a single subject."""
        
        self.logger.info(f"Starting ROI analysis for {subject_id} - {roi_name} ({hemisphere})")
        
        # Get or create ROI mask
        atlas_dir = self.roi_dir / "atlas_masks"
        roi_mask = atlas_dir / f"{roi_name}_{hemisphere}_mask.nii.gz"
        
        if not roi_mask.exists():
            roi_mask = self.download_atlas_masks(roi_name, hemisphere)
            if roi_mask is None:
                return None
        
        # Analyze sustained/phasic responses
